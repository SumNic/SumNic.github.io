...
$result = $mysql->query("SELECT * FROM `date-exp` WHERE `fname` = '$fname'");

$user = $result->fetch_assoc(); // Конвертируем в массив
if (empty($user)) {
    $sql = "INSERT INTO `date-exp` (fname, date1, date2, period) VALUES ('$fname' ,'$date1', '$date2', '$date')";
} else {
$sql = "UPDATE `date-exp` SET date1='$date1',date2='$date2',period='$date' WHERE `fname` = '$fname'";
}
if($mysql->query($sql)){
